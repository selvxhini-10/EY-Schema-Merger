...
import { Upload, Sparkles, CheckCheck, Download, FileText, Info } from "lucide-react"
...
export default function TopBar({ setBankASchema, setBankBSchema }: TopBarProps) {
  ...
  const [bankAFiles, setBankAFiles] = useState<{ name: string, isFolder: boolean, zipEntries?: string[] }[]>([])
  const [bankBFiles, setBankBFiles] = useState<{ name: string, isFolder: boolean, zipEntries?: string[] }[]>([])
  ...
  const handleBankAChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      items.push({ name: isFolder ? file.webkitRelativePath.split("/")[1] : file.name, isFolder, zipEntries });
    }
    setBankAFiles(items);
    ...
    <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold" style={{ color: COLORS.dark }}>Schema Sync</h1>
                  <p className="text-xs" style={{ color: '#64748B' }}>AI-Powered Data Integration</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button 
                variant="outline" 
                size="sm" 
                onClick={handleUploadBankA}
                className="border-2"
                style={{ borderColor: COLORS.primary, color: COLORS.primary }}
              >
                <Upload className="h-4 w-4 mr-2" />
                Upload Bank A Schema
              </Button>
              <Button 
                variant="outline" 
                size="sm" 
                onClick={handleUploadBankB}
                className="border-2"
                style={{ borderColor: COLORS.secondary, color: COLORS.secondary }}
              >
                <Upload className="h-4 w-4 mr-2" />
                Upload Bank B Schema
              </Button>
          ...
          <div className="flex gap-4 mt-4">
            {bankAFiles.length > 0 && (
              <div className="flex-1">
                <div className="text-xs font-semibold mb-1" style={{ color: COLORS.primary }}>
                  Bank A Files ({bankAFiles.length})
                </div>
                <div className="bg-gray-50 rounded p-2 max-h-20 overflow-auto">
                  {bankAFiles.map((item, idx) => (
                    <div key={idx} className="text-xs truncate" style={{ color: '#475569' }}>
            ...
            {bankBFiles.length > 0 && (
              <div className="flex-1">
                <div className="text-xs font-semibold mb-1" style={{ color: COLORS.secondary }}>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-blue-50 border-b">
        <div className="container mx-auto px-6 py-6">
          <Alert className="border-2" style={{ borderColor: COLORS.primary, backgroundColor: 'white' }}>
            <Info className="h-5 w-5" style={{ color: COLORS.primary }} />
            <AlertDescription>
              <div className="space-y-3">
                <h3 className="font-bold text-base" style={{ color: COLORS.dark }}>
                ...
                <div className="grid md:grid-cols-3 gap-4 text-sm">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-semibold" style={{ color: COLORS.dark }}>Upload Master Schemas</span>
                    </div>
                    <p style={{ color: '#64748B' }}>
                      Upload the <strong>master schema file</strong> for Bank A and Bank B. These files should contain all field names, data types, and sample values.
                    </p>
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: COLORS.secondary }}>
                        2
                      </div>
                      <span className="font-semibold" style={{ color: COLORS.dark }}>Upload Additional Files</span>
                    </div>
                    <p style={{ color: '#64748B' }}>
                      Upload any additional data files, transaction records, or supporting documents for each bank separately.
                    </p>
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: '#10B981', color: 'white' }}>
                        3
                      </div>
                      <span className="font-semibold" style={{ color: COLORS.dark }}>AI Mapping & Merge</span>
                    </div>
                    <p style={{ color: '#64748B' }}>
                      Click <strong>"Run AI Mapping"</strong> to automatically match and merge schemas. Review suggestions and approve mappings.
                    </p>
                  </div>
                </div>
              </div>
